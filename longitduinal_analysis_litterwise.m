...
for ii = 1:numel(files)
    ...
    std_low(ii) = std(slopeLow(slopeLow > 0));
    std_high(ii) = std(slopeHigh(slopeHigh > 0));
    std_whole(ii) = std(slopeMap2(slopeMap2 > 0));
...
% labels = ['mean_high'; 'std_high','meanLow','meanWhole','stdWhole','highPerfVol','lowPerfVole','groupFlag','litter']
 array_write = [mean_low; std_low; mean_high; std_high; mean_whole; std_whole; allIds; allages; allflags];
xlswrite(fullfile(outDir,['longitudinal_data__litterwise',date,'_.xlsx']),array_write);
