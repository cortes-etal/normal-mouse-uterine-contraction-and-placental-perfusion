...
outDir = ['compartment_CurveDat_spect' ,date] ;
mkdir(outDir);
...
for ii = 12:15 %:numel(files)
    ...
    for l = 1:numel(labels)
        ...
        if ii == 13
            slopeCurveLow = lowCurve(10:end);
            slopeCurveHigh = highCurve(10:end);
            slopeCurveWhole = wholeCurve(10:end);
            
        end
        %% Spectrogram Calculation
        
        highCurve = (highCurve - mean(highCurve)) ./ rms(highCurve);
        lowCurve = (lowCurve - mean(lowCurve)) ./ rms(lowCurve);
        ...
        plotting_stftDB(stftDB_high,stftDB_low,stftDB_whole,t,f,outDir,saveDir,l);
        %% PSD Estimation
        fs=Fs;
        ...
        per = {SDper_high,SDper_low,SDper_whole};
        welch= {SDwelch_high,SDwelch_low,SDwelch_whole};
        thom = {SDthom_high, SDthom_low, SDthom_whole};
        ar = {SDar_high,SDar_low,SDar_whole};
        
        fper = {Fper_high,Fper_low,Fper_whole};
        fwelch = {Fwel_high,Fwel_low,Fwel_whole};
        fthom = {Fthom_high,Fthom_low,Fthom_whole};
        far = {Far_high,Far_low,Far_whole};
       
        %%% plotting PSDs
        plotting_combinedPSDs(per,welch,thom,ar,fper,fwelch,fthom,far,outDir,saveDir,l);
